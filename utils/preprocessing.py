...
def register_imgset(imgset, mask):
    ...
    for i in range(imgset.shape[-1]):
        x = imgset[...,i]; m = mask[...,i]
        # Unpack the phase_cross_correlation result properly
        s, error, diffphase = phase_cross_correlation(ref, x, reference_mask=m)
        
        # s now contains only the shift tuple
    ...
    # Return the registered images, not the original ones
...
def bicubic(X, scale=3):
    ...
    for i, lr in enumerate(X):
        ...
        sr_img = rescale(lr, scale=scale, order=3, mode='edge',
                         anti_aliasing=False, channel_axis=-1, preserve_range=True) 
        X_upscaled[i] = sr_img
